...
class WechatContact(Base):
    ...
    tags = Column(Text, nullable=True)  # JSON格式存储标签，与CustomerProfile.profile_value.labels同步
    require_approval = Column(Boolean, default=True)  # AI画像更新是否需要批准，False表示直接推送
    tracking_start_date = Column(DateTime, nullable=True)  # 跟踪开始时间
    current_period = Column(Integer, default=1)  # 当前跟踪期数
    period_contact_count = Column(Integer, default=0)  # 当前期已触达次数
    silence_period_count = Column(Integer, default=0)  # 连续沉默期数
    is_silenced = Column(Boolean, default=False)  # 是否已标记为沉默
...
class UserTrackingConfig(Base):
    """用户跟踪配置表"""
    __tablename__ = "user_tracking_configs"
    
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    customer_type = Column(String(20), nullable=False)
    tracking_cycle_days = Column(Integer, default=90)
    tracking_periods = Column(Integer, default=3)
    period_duration_days = Column(Integer, default=30)
    max_contacts_per_period = Column(Integer, default=2)
    contact_interval_days = Column(Integer, default=7)
    silence_threshold_periods = Column(Integer, default=3)
    auto_tracking_enabled = Column(Boolean, default=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
