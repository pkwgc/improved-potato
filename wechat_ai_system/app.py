...
def initial_sync():
    ...
    try:
        ...
        # 判断所有输入是否均为空
        if not (nickname or avatar_style or all_moments_content):
...
{{
  ...
  "hobbies": ["兴趣爱好1", "兴趣爱好2", "兴趣爱好3"]
        ...
        if not contact:
            ...
            logger.debug(f"跳过更新联系人 {wechat_id} 的昵称")
            labels_value = profile_data.get("labels", [])
            if isinstance(labels_value, list) and all(isinstance(label, str) for label in labels_value):
                tags = json.dumps(labels_value, ensure_ascii=False)
                contact.tags = tags
                logger.info(f"成功更新联系人标签: {labels_value}")
            else:
                logger.warning(f"标签数据格式错误: {labels_value}，跳过更新 tags 字段")
        ...
        if not customer_profile:
            ...
            customer_profile.profile_value = profile_value
            customer_profile.value_level = value_level
        ...
        logger.exception(f"朋友圈AI画像分析失败: {str(e)}")
        if db is not None:
            db.rollback()
...
def chat():
    ...
    try:
        ...
        logger.info(f"从[AI 原始回复内容]中提取到JSON: {pretty_json}")
...
def admin_personalized_prompt():
    ...
    try:
        ...
        ai_strategies = db.query(AIStrategy).all()
        ...
        return render_template('admin_personalized_prompt.html', 
                               message=f"加载设置出错: {str(e)}",
                               message_type="danger",
                               all_templates=[],
                               ai_strategies=[])
    finally:
...
@app.route('/api/get_ai_strategy/<int:strategy_id>', methods=['GET'])
def get_ai_strategy(strategy_id):
    """获取单个AI策略信息"""
    try:
        db = next(get_db())
        ...
        logger.error(f"获取AI策略失败: {str(e)}")
        return jsonify({"error": "服务器内部错误"}), 500


@app.route('/api/edit_ai_strategy/<int:strategy_id>', methods=['PUT'])
def edit_ai_strategy(strategy_id):
    """编辑AI策略"""
    try:
        data = request.get_json()
        if not data:
        ...
        existing = db.query(AIStrategy).filter(
            AIStrategy.name == name, 
        ...
        strategy.prompt_template = prompt_template
        strategy.system_message = system_message
        ...
        logger.error(f"编辑AI策略失败: {str(e)}")
        return jsonify({"error": "服务器内部错误"}), 500


@app.route('/api/delete_ai_strategy/<int:strategy_id>', methods=['DELETE'])
def delete_ai_strategy(strategy_id):
    """删除AI策略"""
    try:
        db = next(get_db())
        ...
        contacts_using = db.query(WechatContact).filter(
            WechatContact.ai_strategy_id == strategy_id
        ).count()
        
        if contacts_using > 0:
            return jsonify({"error": f"无法删除：有 {contacts_using} 个联系人正在使用此策略"}), 400
...
def admin_customer_profiling():
    db = next(get_db())
    try:
        value_level = request.args.get('value_level', '').strip()
        activity_min = request.args.get('activity_min', '').strip()
        activity_max = request.args.get('activity_max', '').strip()
        ...
        customers_query = db.query(
            ...
            User.username.label('system_username'),
            User.user_id.label('system_user_id')
        ).join(
            WechatContact, CustomerProfile.contact_id == WechatContact.id
        ).join(
            User, WechatContact.owner_id == User.id
        )
        
        if value_level:
            customers_query = customers_query.filter(CustomerProfile.value_level == value_level)
        
        if activity_min:
            try:
                min_score = float(activity_min) / 100.0
                customers_query = customers_query.filter(CustomerProfile.activity_score >= min_score)
            except ValueError:
                pass
        
        if activity_max:
            try:
                max_score = float(activity_max) / 100.0
                customers_query = customers_query.filter(CustomerProfile.activity_score <= max_score)
            except ValueError:
                pass
        
        if tag_filter:
            customers_query = customers_query.filter(CustomerProfile.profile_value.like(f'%{tag_filter}%'))
        
        if search_keyword:
            search_term = f'%{search_keyword}%'
            customers_query = customers_query.filter(
                or_(
                    WechatContact.nickname.like(search_term),
                    WechatContact.wechat_id.like(search_term),
                    WechatContact.remark.like(search_term),
        ...
        customers = []
        for profile, nickname, wechat_id, remark, system_username, system_user_id in customers_data:
            profile_data = {}
            tags = []
            ...
            customer_data = {
                'id': profile.id,
                'username': remark or nickname or wechat_id,
                'user_id': wechat_id,
                'value_level': profile.value_level or '潜在用户',
                'activity_score': int(profile.activity_score * 100) if profile.activity_score else 0,
                'tags': json.dumps([{'tag': tag} for tag in tags]) if tags else '[]',
                'created_at': profile.created_at,
                'profile_data': profile_data,
                'confidence': profile.confidence or 0.0,
                'conversion_rate': profile.confidence or 0.0,
                'last_active_time': profile.updated_at,
...
def get_admin_contacts():
    ...
    try:
        db = next(get_db())
        contacts = db.query(WechatContact).outerjoin(AIStrategy).all()
        ...
        for contact in contacts:
            contact_list.append({
                ...
                'nickname': contact.nickname,
                'remark': contact.remark,
                'ai_strategy_id': contact.ai_strategy_id,
                'ai_strategy_name': contact.ai_strategy.name if contact.ai_strategy else None
            })
        ...
        logger.error(f"获取联系人列表失败: {str(e)}")
        return jsonify({'error': '获取联系人列表失败'}), 500

@app.route('/api/admin/bind_strategy', methods=['POST'])
@admin_required
def bind_strategy():
    """绑定AI策略到联系人"""
    try:
        ...
        if not contact_ids or not strategy_id:
            return jsonify({"error": "缺少必要参数"}), 400
        ...
        strategy = db.query(AIStrategy).filter(AIStrategy.id == strategy_id).first()
        if not strategy:
            return jsonify({"error": "AI策略不存在"}), 404

        updated_count = 0
        for contact_id in contact_ids:
            contact = db.query(WechatContact).filter(WechatContact.id == contact_id).first()
            if contact:
                contact.ai_strategy_id = strategy_id
        ...
        return jsonify({
            "success": True,
            "message": f"成功绑定 {updated_count} 个联系人到AI策略 '{strategy.name}'"
        })
    except Exception as e:
        logger.error(f"绑定AI策略失败: {str(e)}")
        return jsonify({"error": "服务器内部错误"}), 500

@app.route('/api/admin/unbind_strategy', methods=['POST'])
@admin_required
def unbind_strategy():
    """解除联系人的AI策略绑定"""
    try:
        ...
        return jsonify({
            "success": True,
            "message": f"成功解除 {updated_count} 个联系人的AI策略绑定"
        })
    except Exception as e:
        logger.error(f"解除AI策略绑定失败: {str(e)}")
        return jsonify({"error": "服务器内部错误"}), 500

@app.route('/api/admin/binding_relations', methods=['GET'])
@admin_required
def get_binding_relations():
    """获取当前绑定关系"""
    try:
        db = next(get_db())
        
        contacts_with_strategy = db.query(WechatContact).join(AIStrategy).all()
        
        relations = []
        for contact in contacts_with_strategy:
            relations.append({
                'contact_id': contact.id,
                'contact_name': contact.nickname or contact.remark or contact.wechat_id,
                'wechat_id': contact.wechat_id,
                'strategy_id': contact.ai_strategy_id,
                'strategy_name': contact.ai_strategy.name,
                'bind_time': contact.updated_at.strftime('%Y-%m-%d %H:%M') if contact.updated_at else '未知'
            })
        ...
        logger.error(f"获取绑定关系失败: {str(e)}")
        return jsonify({'error': '获取绑定关系失败'}), 500
...
def user_customer_management():
    ...
    try:
        ...
        for contact in contacts:
            ...
            if contact.tags:
                try:
                    contact.tag_list = json.loads(contact.tags)
                except json.JSONDecodeError:
                    contact.tag_list = []
            else:
                # 从CustomerProfile获取标签并同步到contact.tags
        ...
        for contact in contacts:
            ...
            contact.customer_profiles = customer_profiles
...
@app.route('/user/proactive_tracking')
@user_required
def user_proactive_tracking():
    """用户主动跟踪管理页面 - 仅显示统计数据，不显示AI提示词"""
    if not session.get('user_id'):
        return redirect(url_for('user_login'))
    ...
    try:
        ...
        owner_filter = WechatContact.owner_id == user.id

        # 公共查询基础
        proactive_base = db.query(ProactiveMessage).join(
            WechatContact, ProactiveMessage.contact_id == WechatContact.id
        ).filter(owner_filter)
        ...
        tracking_stats = {
            ...
            'today_sent': proactive_base.filter(
                ProactiveMessage.sent_time >= datetime.now().date()
            ).count(),
            'pending_count': proactive_base.filter(
                ProactiveMessage.status == 'pending'
            ).count(),
        }

        sent_query = proactive_base.filter(ProactiveMessage.status == 'sent')
        replied_query = sent_query.filter(ProactiveMessage.response_received == True)

        sent_messages = sent_query.count()
        replied_messages = replied_query.count()

        tracking_stats['response_rate'] = round(
            (replied_messages / sent_messages * 100) if sent_messages > 0 else 0, 1
        )

        proactive_messages = proactive_base.order_by(
            ProactiveMessage.created_at.desc()
        ).limit(50).all()

        return render_template('user_proactive_tracking.html',
                               tracking_stats=tracking_stats,
                               proactive_messages=proactive_messages,
                               user=user)
    except Exception as e:
        db.rollback()
        # 可选：记录日志
        # logger.error(f"Database error in user_proactive_tracking: {e}")
        return "系统内部错误，请稍后再试", 500
    finally:
        db.close()
...
def get_customer_api(customer_id):
    db = next(get_db())
    try:
        profile = db.query(CustomerProfile).join(
            WechatContact, CustomerProfile.contact_id == WechatContact.id
        ).join(
            User, WechatContact.owner_id == User.id
        ).filter(CustomerProfile.id == customer_id).first()
        
        if not profile:
            return jsonify({'success': False, 'message': '客户画像不存在'})
        ...
        return jsonify({
            ...
            'activity_score': int(profile.activity_score * 100) if profile.activity_score else 0,
            'profile_desc': profile_data.get('summary', ''),
            'tags': json.dumps([{'tag': tag} for tag in tags]) if tags else '[]',
            'wechat_info': {
                'nickname': profile.contact.nickname,
                'wechat_id': profile.contact.wechat_id,
                'remark': profile.contact.remark
            },
            'system_user': {
                'username': profile.contact.owner.username,
                'user_id': profile.contact.owner.user_id
            }
        })
    finally:
        db.close()
...
def update_customer_api(customer_id):
    ...
    try:
        ...
        data = request.get_json()
        profile.value_level = data.get('value_level', profile.value_level)
        
        activity_score = data.get('activity_score', 0)
        if isinstance(activity_score, (int, float)):
            profile.activity_score = float(activity_score) / 100.0
        ...
        if 'profile_desc' in data:
            profile_data['summary'] = data['profile_desc']
        
        if 'tags' in data and data['tags']:
            if isinstance(data['tags'], str):
                try:
                    tags_json = json.loads(data['tags'])
                    if isinstance(tags_json, list):
                        tags_list = [tag['tag'] if isinstance(tag, dict) else str(tag) for tag in tags_json]
                    else:
            ...
            profile_data['labels'] = tags_list
        
        profile.profile_value = json.dumps(profile_data, ensure_ascii=False)
        ...
        log = OperationLog(
            ...
            operation_desc=f'编辑客户画像 {profile.contact.nickname or profile.contact.wechat_id} 的信息',
        ...
        logger.error(f"更新客户画像失败: {str(e)}")
        return jsonify({'success': False, 'message': str(e)})
    finally:
        db.close()

@app.route('/admin/api/customer', methods=['POST'])
@admin_required
def create_customer_api():
    ...
    try:
        ...
        if not contact_id:
            return jsonify({'success': False, 'message': '请选择微信好友'})
        ...
        if existing_profile:
            return jsonify({'success': False, 'message': '该微信好友已有画像记录'})
        
        contact = db.query(WechatContact).filter(WechatContact.id == contact_id).first()
        if not contact:
            return jsonify({'success': False, 'message': '微信好友不存在'})
        
        profile_data = {
            'summary': data.get('profile_desc', ''),
            'labels': []
        }
        ...
        new_profile = CustomerProfile(
            contact_id=contact_id,
            profile_type='综合画像',
            profile_value=json.dumps(profile_data, ensure_ascii=False),
            value_level=data.get('value_level', '潜在用户'),
            activity_score=float(data.get('activity_score', 0)) / 100.0,
            confidence=0.8,
            source='人工'
        )
        ...
        log = OperationLog(
            user_id=None,
            operation_type='客户画像新增',
            operation_desc=f'新增客户画像 {contact.nickname or contact.wechat_id}',
            target_type='CustomerProfile',
        ...
        logger.error(f"创建客户画像失败: {str(e)}")
        return jsonify({'success': False, 'message': str(e)})
    finally:
        db.close()

@app.route('/admin/api/customer/<int:customer_id>', methods=['DELETE'])
@admin_required
def delete_customer_api(customer_id):
    ...
    try:
        ...
        contact_info = f"{profile.contact.nickname or profile.contact.wechat_id}"
        ...
        log = OperationLog(
            user_id=None,
            operation_type='客户画像删除',
            operation_desc=f'删除客户画像 {contact_info}',
            target_type='CustomerProfile',
            target_id=str(customer_id)
        )
        db.add(log)
...
def get_contacts_api():
    ...
    try:
        ...
        contacts_data = []
        for contact in contacts:
            display_name = contact.remark or contact.nickname or contact.wechat_id
            contacts_data.append({
                'id': contact.id,
                'display_name': f"{display_name} ({contact.owner_username})",
                'nickname': contact.nickname,
                'wechat_id': contact.wechat_id,
                'remark': contact.remark,
...
def approve_instruction(message_id):
    ...
    try:
        ...
        if not message:
            return jsonify({'success': True})  # 或者根据需求返回 {'success': False, 'message': '不存在该消息'}

        message.status = 'sent'
        message.sent_time = datetime.now()  # 如果数据库字段允许 NULL，可以先赋值 None 再更新

        try:
            db.commit()
        except Exception as e:
            db.rollback()
            logger.error(f"数据库提交失败: message_id={message_id}, error={str(e)}", exc_info=True)
            return jsonify({'success': False, 'error': '数据库提交失败'})

        if message.message_type == "AI画像更新":
            contact = db.query(WechatContact).filter(WechatContact.id == message.contact_id).first()
            if contact:
                profile = db.query(CustomerProfile).filter(
                    CustomerProfile.contact_id == contact.id,
                    CustomerProfile.profile_type == "综合画像"
                ...
                if profile and profile.profile_value:
                    try:
                        profile_data = json.loads(profile.profile_value)
                        push_profile_update(contact, profile_data)
                    except json.JSONDecodeError as e:
                        logger.error(f"解析画像数据失败: contact_id={contact.id}, error={str(e)}", exc_info=True)
        else:
            try:
                socketio.emit('send_message', {
                ...
                logger.error(f"SocketIO 发送消息失败: message_id={message_id}, error={str(e)}", exc_info=True)

        return jsonify({'success': True})

    except Exception as e:
        logger.error(f"批准指令失败: message_id={message_id}, error={str(e)}", exc_info=True)
...
def user_data_analytics():
    ...
    try:
        ...
        wechat_account_filter = request.args.get('wechat_account')
        value_level_filter = request.args.get('value_level')
        activity_level_filter = request.args.get('activity_level')
        ...
        if wechat_account_filter:
            base_query = base_query.filter(WechatContact.id == wechat_account_filter)
        
        if value_level_filter:
            base_query = base_query.filter(CustomerProfile.value_level == value_level_filter)
        
        if activity_level_filter:
            if activity_level_filter == 'high':
                base_query = base_query.filter(CustomerProfile.activity_score >= 0.7)
            elif activity_level_filter == 'medium':
                base_query = base_query.filter(CustomerProfile.activity_score >= 0.3, CustomerProfile.activity_score < 0.7)
            elif activity_level_filter == 'low':
                base_query = base_query.filter(CustomerProfile.activity_score < 0.3)
        
        if keyword_filter:
            base_query = base_query.filter(
                or_(
                    WechatContact.nickname.like(f'%{keyword_filter}%'),
                    WechatContact.remark.like(f'%{keyword_filter}%'),
                    CustomerProfile.profile_value.like(f'%{keyword_filter}%')
                )
            )
        
        user_profiles = base_query.all()
        
        intent_query = db.query(
            IntentTracking.intent_type,
            func.count(IntentTracking.id).label('count')
        ).filter(IntentTracking.user_id == user.id)
        
        if wechat_account_filter:
            intent_query = intent_query.filter(IntentTracking.contact_id == wechat_account_filter)
        
        intent_stats = intent_query.group_by(IntentTracking.intent_type).all()
        ...
        if wechat_account_filter:
            user_type_query = user_type_query.filter(WechatContact.id == wechat_account_filter)
        
        if value_level_filter:
            user_type_query = user_type_query.filter(CustomerProfile.value_level == value_level_filter)
        
        user_type_stats = user_type_query.group_by(CustomerProfile.value_level).all()
        ...
        if wechat_account_filter:
            activity_query = activity_query.filter(WechatContact.id == wechat_account_filter)
        
        activity_stats = activity_query.first()
        
        user_wechat_accounts = db.query(WechatContact).filter(
            WechatContact.owner_id == user.id,
            WechatContact.is_group == False,
            WechatContact.parent_id.is_(None)
        ).all()
        
        filtered_profiles = []
        for profile, nickname, wechat_id, remark in user_profiles:
            latest_intent = db.query(IntentTracking).filter(
                IntentTracking.contact_id == profile.contact_id
            ).order_by(IntentTracking.detected_at.desc()).first()
            ...
            profile_data = {
                ...
                'latest_intent': latest_intent.intent_type if latest_intent else None,
                'owner_wechat_id': wechat_id
            }
        ...
        return render_template('user_data_analytics.html',
                             ...
                             user_wechat_accounts=user_wechat_accounts,
...
def get_intent_distribution():
    ...
    try:
        ...
        intent_stats = db.query(
            IntentTracking.intent_type,
            func.count(IntentTracking.id).label('count')
        ).filter(IntentTracking.user_id == user.id).group_by(IntentTracking.intent_type).all()
...
@app.route('/user/api/contact/<int:contact_id>/auto-follow', methods=['POST'])
@user_required
def toggle_auto_follow(contact_id):
    """切换联系人自动跟踪状态"""
    db = next(get_db())
    try:
        user_id = session.get('user_id')
        ...
        if not contact:
            return jsonify({'error': '联系人不存在或无权访问'}), 404
        
        data = request.get_json()
        auto_follow_enabled = data.get('auto_follow_enabled', False)
        
        old_status = contact.auto_follow_enabled
        contact.auto_follow_enabled = auto_follow_enabled
        
        if not old_status and auto_follow_enabled:
            try:
                profile_result = trigger_ai_profiling(db, contact)
                
                if profile_result and profile_result.get('labels'):
                    contact.tags = json.dumps(profile_result['labels'], ensure_ascii=False)
                    ...
                    if existing_profile:
                        existing_profile.profile_value = json.dumps(profile_result, ensure_ascii=False)
                        existing_profile.value_level = profile_result.get("category", "普通客户")
                ...
                if contact.require_approval:
                    create_profile_approval_message(db, contact, profile_result)
                else:
                    push_profile_update(contact, profile_result)
                
            except Exception as e:
                logger.error(f"触发AI画像生成失败: {str(e)}")
        
        db.commit()
        
        return jsonify({
            'success': True,
            'auto_follow_enabled': contact.auto_follow_enabled,
            'message': '跟踪状态更新成功'
        })
        
    except Exception as e:
        logger.error(f"更新跟踪状态失败: {str(e)}")
        return jsonify({'error': '更新失败'}), 500
    finally:
        db.close()

@app.route('/user/api/contact/<int:contact_id>/tags', methods=['POST'])
@user_required
...
@app.route('/user/api/contact/<int:contact_id>/approval-setting', methods=['POST'])
@user_required
def update_approval_setting(contact_id):
    """更新联系人AI画像批准设置"""
    db = next(get_db())
    try:
        user_id = session.get('user_id')
        ...
        contact.require_approval = require_approval
        db.commit()
        
        return jsonify({
            'success': True,
            'require_approval': contact.require_approval,
            'message': '批准设置更新成功'
        })
        
    except Exception as e:
        logger.error(f"更新批准设置失败: {str(e)}")
        return jsonify({'error': '更新失败'}), 500
    finally:
        db.close()

def trigger_ai_profiling(db: Session, contact: WechatContact):
    """触发AI画像生成"""
    try:
        moments = db.query(Moment).filter(
            Moment.wechat_id == contact.wechat_id
        ).order_by(Moment.created_time.desc()).limit(20).all()
        
        if not moments:
            return {"summary": "", "labels": [], "category": "普通客户", "hobbies": []}
        
        all_moments_content = []
        for moment in moments:
            moment_text = f"时间: {moment.created_time.strftime('%Y-%m-%d %H:%M')}\n"
            moment_text += f"内容: {moment.content or '图片/视频内容'}\n"
            if moment.like_count > 0:
                moment_text += f"点赞数: {moment.like_count}\n"
            if moment.comment_count > 0:
                moment_text += f"评论数: {moment.comment_count}\n"
            all_moments_content.append(moment_text)
        
        template_content = ""
        if contact.follow_template_id:
            template = db.query(Template).filter(Template.id == contact.follow_template_id).first()
            if template:
                template_content = template.content
...
以下是用户最近的朋友圈动态：
{chr(10).join(all_moments_content)}
...
{{
  ...
  "hobbies": ["兴趣爱好1", "兴趣爱好2", "兴趣爱好3"]
        ...
        logger.error(f"AI画像生成失败: {str(e)}")
        return {"summary": "", "labels": [], "category": "普通客户", "hobbies": []}

def create_profile_approval_message(db: Session, contact: WechatContact, profile_result: dict):
    """创建AI画像更新的批准消息"""
    try:
        content = f"AI为联系人 {contact.nickname or contact.wechat_id} 生成了新的画像分析：\n\n"
        content += f"摘要：{profile_result.get('summary', '无')}\n"
        content += f"标签：{', '.join(profile_result.get('labels', []))}\n"
        content += f"分类：{profile_result.get('category', '普通客户')}\n"
        content += f"兴趣：{', '.join(profile_result.get('hobbies', []))}\n\n"
        content += "批准后将推送画像更新到对应房间。"
        ...
        logger.info(f"已创建AI画像批准消息: 联系人ID={contact.id}")
        return True
        
    except Exception as e:
        logger.error(f"创建AI画像批准消息失败: {str(e)}")
        return False

def push_profile_update(contact: WechatContact, profile_result: dict):
    """推送画像更新到WebSocket"""
    try:
        from socketio_server import broadcast_to_room
        from datetime import datetime
        
        room_name = f"wechat_{contact.wechat_id}"
        message_data = {
            'id': f'profile_update_{int(datetime.now().timestamp() * 1000)}',
            'type': 'profile_update',
            'content': {
                'contact_id': contact.id,
                'contact_name': contact.nickname or contact.wechat_id,
                'profile_data': profile_result,
                'tags': profile_result.get('labels', [])
            },
            'sender': {'id': 'system', 'name': 'AI画像系统'},
            'receiver': {'id': contact.wechat_id, 'name': contact.nickname or contact.wechat_id},
            'timestamp': datetime.now().isoformat(),
            'require_ack': True
        }
        
        broadcast_to_room(room_name, 'new_message', message_data)
        logger.info(f"推送画像更新到房间: {room_name}")
        
    except Exception as e:
        logger.error(f"推送画像更新失败: {str(e)}")
...
@app.route('/user/api/tracking-config', methods=['GET'])
@user_required
def get_user_tracking_config():
    """获取用户跟踪配置"""
    db = next(get_db())
    try:
        user_id = session.get('user_id')
        ...
        configs = db.query(UserTrackingConfig).filter(
            UserTrackingConfig.user_id == user.id
        ).all()
        
        config_dict = {}
        for config in configs:
            config_dict[config.customer_type] = {
                'tracking_cycle_days': config.tracking_cycle_days,
                'tracking_periods': config.tracking_periods,
                'period_duration_days': config.period_duration_days,
                'max_contacts_per_period': config.max_contacts_per_period,
                'contact_interval_days': config.contact_interval_days,
                'silence_threshold_periods': config.silence_threshold_periods,
                'auto_tracking_enabled': config.auto_tracking_enabled
            }
        
        return jsonify({'success': True, 'configs': config_dict})
        
    except Exception as e:
        logger.error(f"获取用户跟踪配置失败: {str(e)}")
        return jsonify({'error': '获取配置失败'}), 500
    finally:
        db.close()

@app.route('/user/api/tracking-config', methods=['POST'])
@user_required
def save_user_tracking_config():
    """保存用户跟踪配置"""
    db = next(get_db())
    try:
        user_id = session.get('user_id')
        ...
        if not customer_type:
            return jsonify({'error': '客户类型不能为空'}), 400
        
        config = db.query(UserTrackingConfig).filter(
            UserTrackingConfig.user_id == user.id,
            UserTrackingConfig.customer_type == customer_type
        ).first()
        ...
        config.tracking_cycle_days = data.get('tracking_cycle_days', 90)
        config.tracking_periods = data.get('tracking_periods', 3)
        config.period_duration_days = data.get('period_duration_days', 30)
        config.max_contacts_per_period = data.get('max_contacts_per_period', 2)
        config.contact_interval_days = data.get('contact_interval_days', 7)
        config.silence_threshold_periods = data.get('silence_threshold_periods', 3)
        config.auto_tracking_enabled = data.get('auto_tracking_enabled', True)
        ...
        log_entry = OperationLog(
            user_id=user.id,
            operation_type='跟踪配置更新',
            operation_desc=f'更新{customer_type}跟踪配置',
        ...
        logger.error(f"保存用户跟踪配置失败: {str(e)}")
        return jsonify({'error': '保存配置失败'}), 500
    finally:
        db.close()

@app.route('/user/api/contact/<int:contact_id>/manual-follow', methods=['POST'])
@user_required
def manual_follow_contact(contact_id):
    """手动触发联系人跟踪"""
    db = next(get_db())
    try:
        user_id = session.get('user_id')
        ...
        if not user:
            logger.error(f"手动跟踪失败: 用户不存在 user_id={user_id}")
            return jsonify({'error': '用户不存在'}), 404
        ...
        if not contact:
            logger.error(f"手动跟踪失败: 联系人不存在或无权访问 contact_id={contact_id}, user_id={user.id}")
            return jsonify({'error': '联系人不存在或无权访问'}), 404
        
        data = request.get_json()
        message_type = data.get('message_type', '关怀')
        
        logger.info(f"开始手动跟踪: contact_id={contact_id}, user_id={user.id}, message_type={message_type}")
        
        from proactive_messaging import ProactiveMessagingService
        service = ProactiveMessagingService()
        
        success = service.schedule_proactive_message(db, contact_id, message_type, manual_trigger=True)
        
        if success:
            generated_message = service.generate_personalized_message(db, contact_id, message_type)

            ai_strategy_used = contact.ai_strategy_id is not None
            
            log_entry = OperationLog(
                user_id=user.id,
                operation_type='手动跟踪触发',
                operation_desc=f'手动触发联系人 {contact.nickname or contact.wechat_id} 的跟踪消息，使用{"AI策略" if ai_strategy_used else "预设模板"}',
            ...
            logger.info(f"手动跟踪成功: contact_id={contact_id}, ai_strategy_used={ai_strategy_used}")
            return jsonify({
                'success': True, 
                'message': '跟踪消息已安排发送',
                'ai_strategy_used': ai_strategy_used,
                'generated_message_preview': generated_message[:50] + "..." if len(generated_message) > 50 else generated_message
            })
        else:
            logger.error(f"ProactiveMessagingService.schedule_proactive_message 返回 False: contact_id={contact_id}")
            return jsonify({'error': '安排跟踪消息失败'}), 500
        
    except Exception as e:
        import traceback
        logger.error(f"手动跟踪失败: {str(e)}")
        logger.error(f"手动跟踪异常详情: {traceback.format_exc()}")
        return jsonify({'error': '手动跟踪失败'}), 500
    finally:
        db.close()

@app.route('/user/api/contact/<int:contact_id>/ai-strategy', methods=['GET', 'POST'])
@user_required
def manage_contact_ai_strategy(contact_id):
    """管理联系人AI策略绑定"""
    db = next(get_db())
    try:
        user_id = session.get('user_id')
        ...
        if request.method == 'GET':
            ...
            if contact.ai_strategy_id:
                current_strategy = db.query(AIStrategy).filter(AIStrategy.id == contact.ai_strategy_id).first()
            
            available_strategies = db.query(AIStrategy).all()
            
            return jsonify({
                'current_strategy': {
                    'id': current_strategy.id if current_strategy else None,
                    'name': current_strategy.name if current_strategy else None,
                    'description': current_strategy.description if current_strategy else None
                },
            ...
            log_entry = OperationLog(
                user_id=user.id,
                operation_type='AI策略管理',
                operation_desc=f'联系人 {contact.nickname or contact.wechat_id} {action}',
                target_type='WechatContact',
                target_id=str(contact_id)
            )
            db.add(log_entry)
