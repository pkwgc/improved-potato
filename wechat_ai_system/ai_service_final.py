...
class AIService:
    ...
    def process_chat(self, db: Session, user_id: str, message: str, template_name: Optional[str] = None, 
                    context_id: Optional[str] = None, image_url: Optional[str] = None) -> Dict[str, Any]:
        """处理用户消息，调用AI接口获取回复 (兼容app.py中的process_chat调用)"""
        return self.process_message(db, user_id, message, template_name, context_id, image_url)
