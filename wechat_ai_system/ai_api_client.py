...
class AIServiceClient:
    ...
    def process_chat(self, db, user_id: str, user_message: str,
        ...
        # 参数校验
        if not user_id or not user_message or not template_name:
            logger.error(f"参数缺失: user_id={user_id}, template_name={template_name}")
            return {"error": "参数缺失", "trigger_api": False}
        ...
        try:
            ...
            if result is None:
                logger.error("响应中未包含 'result' 字段", extra={"response": response.text})
                return {"error": "无效响应格式", "trigger_api": False}

            return result

        except requests.exceptions.RequestException as e:
            logger.error(f"AI服务调用失败: {str(e)}", exc_info=True,
                         extra={"user_id": user_id, "template_name": template_name})
            return {"error": "AI服务暂时不可用，请稍后再试", "trigger_api": False}
