...
def user_proactive_tracking():
    ...
    try:
        ...
        tracking_stats = {
            ...
            'today_sent': db.query(ProactiveMessage).join(
                WechatContact, ProactiveMessage.contact_id == WechatContact.id
            ).filter(
                WechatContact.owner_id == user.id,
                ProactiveMessage.sent_time >= datetime.now().date()
            ).count(),
            'pending_count': db.query(ProactiveMessage).join(
                WechatContact, ProactiveMessage.contact_id == WechatContact.id
            ).filter(
                WechatContact.owner_id == user.id,
                ProactiveMessage.status == 'pending'
        ...
        sent_messages = db.query(ProactiveMessage).join(
            WechatContact, ProactiveMessage.contact_id == WechatContact.id
        ).filter(
            WechatContact.owner_id == user.id,
        ...
        replied_messages = db.query(ProactiveMessage).join(
            WechatContact, ProactiveMessage.contact_id == WechatContact.id
        ).filter(
            WechatContact.owner_id == user.id,
            ProactiveMessage.status == 'sent',
            ProactiveMessage.response_received == True
        ).count()
        ...
        proactive_messages = db.query(ProactiveMessage).join(
            WechatContact, ProactiveMessage.contact_id == WechatContact.id
        ).filter(
            WechatContact.owner_id == user.id
        ).order_by(ProactiveMessage.created_at.desc()).limit(50).all()
                ...
                if contact:
                    ...
                    if profile and profile.profile_value:
                        try:
                            ...
                            logger.error(f"解析画像数据失败: contact_id={contact.id}")
            else:
                socketio.emit('send_message', {
    ...
    try:
        ...
        if success:
            log_entry = OperationLog(
                ...
                operation_desc=f'手动触发联系人 {contact.nickname or contact.wechat_id} 的跟踪消息',
            ...
            logger.info(f"手动跟踪成功: contact_id={contact_id}")
            return jsonify({'success': True, 'message': '跟踪消息已安排发送'})
        else:
